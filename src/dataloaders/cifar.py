...
def get(seed=0, pc_valid=0.10, tasknum=10):
    ...
    for i in range(1, 11):
        ...
        for s in ["train", "test"]:
            data[i][s] = {"x": [], "y": []}
            data[i][s]["x"] = torch.load(os.path.join(os.path.expanduser("../dat/binary_split_cifar100"), "data" + str(ids[i - 1]) + s + "x.bin"))
            data[i][s]["y"] = torch.load(os.path.join(os.path.expanduser("../dat/binary_split_cifar100"), "data" + str(ids[i - 1]) + s + "y.bin"))
